...
class ShareQrcodeForm extends Model
{
    ...
    {
        return \Yii::$app->controller->version ? \Yii::$app->controller->version : '1.7.0';
    }
    ...
    public function goods_qrcode()
    {
        ...
        $goods_pic_save_name = md5("v=2.2.3&goods_id={$goods->id}&goods_name={$goods->name}&store_name={$store->name}&user_id={$this->user_id}&goods_pic_url={$goods_pic_url}") . '.jpg';
    ...
    public function ms_goods_qrcode()
    {
        ...
        $goods_pic_save_path = \Yii::$app->basePath . '/web/temp/';
        $goods_pic_save_name = md5("v=2.2.3&f=miaosha&goods_id={$miaosha_goods->id}&goods_name={$goods->name}&store_name={$store->name}&user_id={$this->user_id}&goods_pic_url={$goods_pic_url}") . '.jpg';
...
//        $editor->blend($goods_qrcode, $wxapp_qrcode, 'normal', 1.0, 'top-left', 470, 1040);
    ...
    public function share_qrcode()
    {
        ...
        $save_name = md5("v={$this->getVersion()}&store_id={$this->store_id}&user_id={$this->user->id}") . '.jpg';
        ...
        $scene = "{$this->user_id}";
        $wxapp_qrcode_file_res = $this->getQrcode($scene, $width);
        if ($wxapp_qrcode_file_res['code'] == 1) {
            unlink($qrcode_bg);
            return [
                'code' => 1,
        ...
        //获取小程序码
        if (isset($qrcode_size['c']) && $qrcode_size['c']) {
            $wxapp_qrcode_file_path = $this->test($wxapp_qrcode_file_path, $save_root, $width, $width);
        }
