...
class OrderBehavior extends BaseBehavior
{
    ...
    public function beforeAction($e)
    {
        ...
        */
        ...
        //查如上会员的总消费金额
        $userWithMoney = (new Query)->from(['u' => $userIds])->select([
            'u.*', 
        ...
        //查对应金额的会员等级
        $userWithLevel = (new Query)->from(['u' => $userWithMoney])->select([
            'u.*', 
            'new_level' => Level::find()
                ->select('level')
                ->where(['store_id' => $this->store_id, 'is_delete' => 0, 'status' => 1])
                ->andWhere('money <= u.order_money')
        ...
        $userList = User::find()
            ->alias('u')
            ->select(['u.*', 'uids.*'])
            ->rightJoin(['uids' => $userWithLevel], 'uids.user_id = u.id')
            ->andWhere('uids.order_money is not null')
            ->andWhere('uids.new_level is not null')
            ->all();

        //调整会员等级
        foreach ($userList as $index => $user) {
            if ($user->level < $user->new_level) {
